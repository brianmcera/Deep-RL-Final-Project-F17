...
def reset(pub_cmd,rate):
    reset_cmd_msg = LaikaCommand()
    reset_cmd_msg.header.stamp = rospy.Time.now()
    reset_cmd_msg.cmd = 'reset'
    pub_cmd.publish(reset_cmd_msg)
    print('Reset sent')
    # rate.sleep()
    obs = curr_state
...
def plot_comparison(dyn_model, pub_act, pub_cmd, rate):
    ...
    # plot_states = np.arange(0,108,6)
    plot_states = range(0,12)
...
def train(pub_cmd,pub_act,rate,
    ...
    for itr in range(onpol_iters):
        ...
        dyn_model.fit(data)
        # plot_comparison(dyn_model,pub_act,pub_cmd,rate)
...
def main():
    ...
    # Data collection
    parser.add_argument('--random_paths', '-r', type=int, default=5)
    ...
    pub_cmd = rospy.Publisher('cmd', LaikaCommand, queue_size=10)
